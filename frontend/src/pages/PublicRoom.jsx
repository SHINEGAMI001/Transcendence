  ...
  return (
    <div className="min-h-screen text-text-primary flex flex-col relative overflow-hidden">
      ...
      <footer className="h-24 border-t border-white/10 bg-black/80 backdrop-blur-2xl relative z-20 flex items-center px-12 justify-between">
         ...
         <div className="flex items-center gap-4">
            <button className="px-8 py-4 rounded-xl bg-green-500/20 text-green-400 border border-green-400/30 font-black italic tracking-tighter text-lg hover:bg-green-500/30 transition-all cursor-pointer">
              CREATE ROOM
            </button>
            <button 
              disabled={!selectedRoom || selectedRoom.players === selectedRoom.maxPlayers}
              className={`px-12 py-4 rounded-xl font-black italic tracking-tighter text-lg transition-all duration-300 ${
                selectedRoom && selectedRoom.players < selectedRoom.maxPlayers
